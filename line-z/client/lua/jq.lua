...
if isTimer(scrolling) then --if a text is currently scrolling
	...
	if nT < 4 then --if theres less than 5 text items
		nT = nT + 1 --count up
	...
	theShadow[nT] = guiCreateLabel(1, 0.3, 0, 0, string.gsub(text, '#%x%x%x%x%x%x', ''), true) --create a shadow
	theTexts[nT] = guiCreateLabel(1, 0.3, 0, 0, string.gsub(text, '#%x%x%x%x%x%x', ''), true) --create text
	...
	local fx = dxGetTextWidth(string.gsub(guiGetText(theTexts[nT]), '#%x%x%x%x%x%x', ''),1.2,"default-bold") --get the lenght of the text item
...
function startScroll () --scroll function
	...
	local x = dxGetTextWidth(string.gsub(guiGetText(theTexts[nT]), '#%x%x%x%x%x%x', ''),1.2,"default-bold") --get the lenght of the text
	...
	guiSetSize(theShadow[nT], scrollsize, dxGetFontHeight(1.2,"default-bold"), false) --set the size
	guiSetSize(theTexts[nT], scrollsize, dxGetFontHeight(1.2,"default-bold"), false)
...
function remotePlayerQuit(reason)
	displayGUItextToAll(getPlayerName(source).."#FFFFFF, left ["..reason.."]!", 255, 255, 255, "quit")
end
addEventHandler("onClientPlayerQuit", getRootElement(), remotePlayerQuit)

function remotePlayerQuit(reason)
	displayGUItextToAll("rgrttt#FF9900rrezzz", 255, 255, 255,"join")
end
...
if fadeOut[value] then
	...
	guiSetPosition(value,x,y-0.0008,true)
	if alphaFade[value] >= 1 then
		alphaFade[value] = alphaFade[value]-10.2
	else
	alphaFade[value] = 0
	fadeOut[value] = nil
	end
...
local widht = dxGetTextWidth(string.gsub(guiGetText(value), '#%x%x%x%x%x%x', ''),1.2,"default-bold")
local height = dxGetFontHeight(1.2,"default-bold")
if img then
	if fileExists("client/image/tools/flags/"..img..".png") then		
		dxDrawImage(screenWidth*0.01, y*1.8-height*0.8, screenWidth*0.02,height*0.8,"client/image/tools/flags/"..tostring(img)..".png", 0,0,0,tocolor(255,255,255,alphaFade[value]), false)
	end
end
--Info Text
	guiSetAlpha(value, 0)
	dxDrawingColorText(string.gsub(guiGetText(value), '#%x%x%x%x%x%x', ''),screenWidth*0.04+1, y*1.8+1, screenWidth-screenWidth*0.04+1, y*1.8+1, tocolor(0,0,0,alphaFade[value]), alphaFade[value], 1.2, "default-bold", "left", "bottom", false, false, false )
	dxDrawingColorText(getElementData(value,"realtext"),screenWidth*0.04, y*1.8, screenWidth*0.04, y*1.8, tocolor(255,255,255,alphaFade[value]), alphaFade[value], 1.2, "default-bold", "left", "bottom", false, false, false )
end
end
)
